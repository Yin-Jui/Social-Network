...
class Message{
	...
	public function getMostRecentUser(){
		...
		if($user_to != $userLoggedIn)
			...
			return $user_from; // return the person you talk to
	}
	...
	public function getConvos(){
		...
		$userLoggedIn = $this->user_obj->getUsername();
		$return_string = "";
		$convos = array(); //put users we have conversation with into this array
	...
	public function getConvosDropdown($data, $limit) {
		...
		foreach($convos as $username) {
			...
			$style = (isset($row['opened'])&&$row['opened'] == 'no') ? "background-color: #DDEDFF;" : ""; //unread messages will be highlighted
		...
		if($count > $limit)
			$return_string .= "<input type='hidden' class='nextPageDropdownData' value='" . ($page + 1) . "'><input type='hidden' class='noMoreDropdownData' value='false'>"; //load more post
		else 
