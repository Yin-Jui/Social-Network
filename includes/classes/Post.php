	...
	class Post{
		...
		public function loadPostsFriends($data, $limit){
			...
			if(mysqli_num_rows($data_query) > 0){
				...
				$count = 1; //how many results have been loaded
				...
				while($row = mysqli_fetch_array($data_query)){
					...
					if($user_logged_obj->isFriend($added_by)){
						...
						if($userLoggedIn == $added_by) $delete_button = "<button class = 'delete_button btn-danger' id='post$id'>X</button>";
						else $delete_button = "";
		...
		public function loadProfilePosts($data, $limit){
			...
			//on the profile page, don't appear user to others page
			if(mysqli_num_rows($data_query) > 0){
		...
		else { // no post found in mysqli_num_rows($data_query)
